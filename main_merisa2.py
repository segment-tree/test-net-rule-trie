...
def find_min_index_marisa(trie: marisa_trie.RecordTrie, query_string: str):
    # marisa_trie.RecordTrie.prefixes returns list of matching prefixes (strings)
    prefixes = trie.prefixes(query_string)
    if not prefixes:
        return None
    
    # choose the prefix whose stored index is minimal
    # trie[p] returns a list of tuples like [(index,)]
    best_idx = None
    for p in prefixes:
        # We expect exactly one value per key because we built it that way
        vals = trie[p]
        if vals:
            idx = vals[0][0]
...
def main():
    ...
    trie_items = [(k, (v,)) for k, v in key_to_index.items()]
    trie = marisa_trie.RecordTrie('<I', trie_items)
    ...
    for j in range(m):
        ...
        index = find_min_index_marisa(trie, query_bitstr)
